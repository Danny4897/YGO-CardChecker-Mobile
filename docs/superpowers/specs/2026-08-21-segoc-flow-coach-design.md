...
**Extraction script**: new pure module `ygo-card-checker/tools/card-knowledge-db/src/segoc-parser.ts` (`parseSegocProfile(lua: string): SegocProfile`), following `mdpro-lua-parser.ts`'s existing structural-regex style (per-effect-block scanning is new — today's `parseMdproLua` regexes whole-file — the SEGOC fields are the first thing in this pipeline that need per-block scoping, since a card can mix "when"/"if" triggers or Trigger+Ignition blocks). Wired into the **existing** whole-catalog loop in `build-effect-scripts.ts` (it already opens the local SQLite `cards` table, iterates every card, and loads that card's Lua via `loadMdproLua(card.id)` from the `mdpro-scripts/` cache, 13,523 files already on disk) — no new orchestration script or second whole-catalog pass needed, no change to how `import-mdpro-scripts.ts` invokes it (`npm run db:effect-scripts` already runs this loop). The new pass writes one additional output file, `segoc-profiles.json`, alongside the existing `scripts.json`/`hat-2014.json` in `src/assets/data/effect-scripts/`, keyed by card passcode.
