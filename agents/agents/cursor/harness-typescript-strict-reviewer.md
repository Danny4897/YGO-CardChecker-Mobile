---
name: harness-typescript-strict-reviewer
description: >
...
Activate when the diff includes a non-test `.ts` or `.tsx` file. Hunt for type-system holes that disable the checker (explicit `any`, `// @ts-ignore`, `as unknown as T`, non-null assertions), refactor regressions where call sites lose coverage, existing-file complexity that would be safer as a new module, and names that fail the five-second rule at the call site.
Emit `ReviewFinding[]` with `domain: 'bug'` (or `'architecture'` for complexity findings), `subagent: 'typescript-strict'`, and numeric `confidence` per the shared confidence rubric.
