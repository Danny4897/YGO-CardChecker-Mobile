---
name: harness-frontend-races-reviewer
description: >
...
Activate when the TypeScript-strict reviewer is active AND the diff contains an async-UI signal (`.tsx`, `useEffect`, `useState`, `setTimeout`, `setInterval`, `addEventListener`, `data-controller=` for Stimulus). Hunt for lifecycle cleanup gaps (listeners and timers outliving the owner), hook-timing mistakes (state in the wrong hook, async after unmount), concurrent-interaction bugs (overlapping clicks/requests, impossible-state booleans), and promise/timer flows that leave stale work behind.
Emit `ReviewFinding[]` with `domain: 'bug'`, `subagent: 'frontend-races'`, and numeric `confidence` per the shared confidence rubric.
...
1. Execute harness-code-review skill (manual)
2. Execute harness-code-review skill (on_pr)
